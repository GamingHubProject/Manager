...
Azuriom is downloaded directly from the official Azuriom GitHub release. Its original project files, name, copyright notices, and `LICENSE` file remain intact.

Gaming Hub Manager is installed only under:

```text
plugins/gaming-hub-manager
```
...
Paste this complete block into a Fish terminal:
...
cp -a \
    "$TMP_DIR/manager/gaming-hub-manager" \
    plugins/gaming-hub-manager
...
sudo find "$INSTALL_DIR" \
    -type d \
    -exec setfacl -m "d:u:$WWW_UID:rwX" '{}' +
...
echo
echo "After Azuriom setup:"
echo "Administration -> Extensions -> Plugins -> enable Gaming Hub Manager."
echo
...
Paste this complete block into a Bash terminal:
...
while true; do
    ...
    if command -v ss >/dev/null 2>&1 \
        && ss -ltnH "sport = :${APP_PORT}" 2>/dev/null | grep -q .
    then
        echo "Port ${APP_PORT} is already in use. Choose another port."
        continue
    fi
...
[[ -n "$ARTISAN_PATH" ]] \
    || fail "The official Azuriom archive did not contain artisan."
...
[[ -f docker-compose.yml ]] \
    || fail "docker-compose.yml is missing after extraction."

[[ -f docker/nginx.conf ]] \
    || fail "docker/nginx.conf is missing after extraction."

[[ -f .env.example ]] \
    || fail ".env.example is missing after extraction."

[[ -f LICENSE ]] \
    || fail "The official Azuriom LICENSE file is missing after extraction."
...
do
    grep -qF "$required" docker-compose.yml \
        || fail "Could not apply required Compose setting: $required"
done
...
[[ -f "$TMP_DIR/manager/gaming-hub-manager/plugin.json" ]] \
    || fail "The Manager archive has an unexpected structure."
...
[[ "$WWW_UID" =~ ^[0-9]+$ ]] \
    || fail "Could not determine the container www-data UID."
...
Use these values in the Azuriom database installer:
...
After completing the normal Azuriom setup, open:
...
Use `sudo docker compose` instead when your user does not have direct Docker access.
